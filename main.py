...
@interactions.slash_command(name="fact", description="Get a random fun fact.")
async def random_fact(ctx: interactions.ComponentContext):
    """
    Fetches a random useless fact from Useless Facts API.
    """
    try:
        await ctx.defer()

        fact_url = "https://uselessfacts.jsph.pl/random.json?language=en"
        logger.debug(f"Fetching fact from {fact_url}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(fact_url) as response:
                if response.status == 200:
                    ...
                    logger.warning(f"Useless Facts API error: {response.status}")
                    await ctx.send("🤔 Couldn't fetch a fact. Try again later.")
    except Exception as e:
