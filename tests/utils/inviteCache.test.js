...
describe('inviteCache', () => {
  ...
  it('sets and gets invite snapshot', () => {
  ...
  it('updates snapshot from invite collection', () => {
    const collection = {
      each: (fn) => {
  ...
  it('does nothing when code is not found in snapshot', () => {
    setInviteSnapshot(guildId, { abc: 5 });
    removeInviteFromSnapshot(guildId, 'not-in-snapshot');
    expect(getInviteSnapshot(guildId)).toEqual({ abc: 5 });
  });

  it('does nothing when removing from missing snapshot', () => {
